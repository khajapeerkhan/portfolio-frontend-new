...
export default function Home({ personalInfo, experiences, skills, projects, education, certifications }) {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <section className="flex flex-col items-center gap-4 text-center">
          <div className="w-48 h-48 rounded-full overflow-hidden shadow-xl border-4 border-gray-200">
            <img
              src={personalInfo?.profileImage ? urlFor(personalInfo.profileImage).width(500).height(500).url() : "/default-profile.png"}
          ...
          <div className="mt-3 flex gap-4 justify-center">
            {personalInfo?.linkedin && <a href={personalInfo.linkedin} target="_blank" className="text-blue-600 hover:underline">LinkedIn</a>}
            {personalInfo?.github && <a href={personalInfo.github} target="_blank" className="text-gray-800 hover:underline">GitHub</a>}
          </div>
        </section>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {projects.map((proj, idx) => (
              <div key={idx} className="bg-white shadow-md p-4 rounded-lg">
                ...
                {proj.image && <img src={urlFor(proj.image).url()} alt={proj.title} className="rounded-lg w-full max-h-48 object-cover mb-2" />}
                <p className="text-gray-700 text-sm mb-2">{proj.description}</p>
                <p className="text-gray-500 text-sm mb-2">Tech: {proj.techStack.join(", ")}</p>
                <div className="flex gap-4">
                  {proj.github && <a href={proj.github} target="_blank" className="text-blue-600 hover:underline text-sm">GitHub</a>}
                  {proj.demo && <a href={proj.demo} target="_blank" className="text-green-600 hover:underline text-sm">Live Demo</a>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {skills.map((skill, idx) => (
              ...
              >
                ...
                <span className="mt-1 text-sm text-gray-500">{skill.level}</span>
              </div>
            ))}
          </div>
        </section>
